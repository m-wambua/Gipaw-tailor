...
int get totalCompletedQuantity{
  ...
  bool get isPartiallyCompleted{
    return completedItems.isNotEmpty && totalCompletedQuantity<totalOrderQuantity;
  }
  ...
  void markAsCompleted(List<UniformOrderItem> completed,{bool isfullyCompleted=false}){
...
class UniformOrderItem{
  ...
  factory UniformOrderItem.fromMap(Map<String,dynamic> map){
    return UniformOrderItem(
      uniformName: map['selectedUnifromItem'] ?? 'Unknown Item',
...
class OrdersProvider with ChangeNotifier{
  ...
  List<UniformOrder> get pendingOrders=> _orders.where((order)=> order.status==OrderStatus.pending|| order.status==OrderStatus.partiallyCompleted).toList();


  List<UniformOrder> get completedPendingApprovalOrders=>_orders.where((order)=> order.status==OrderStatus.completedPendingApproval).toList();

  List<UniformOrder> get approvedAndVerifiedOrders=>_orders.where((order)=> order.status==OrderStatus.approvedAndVerified).toList();
  ...
  void markOrderAsCompleted(String orderId, List<UniformOrderItem> completedItems, { bool isfullCompleted=false}){
    final orderIndex=_orders.indexWhere((order)=> order.id==orderId);
    if(orderIndex!=-1){
      _orders[orderIndex].markAsCompleted(completedItems, isfullyCompleted: isfullCompleted);
      notifyListeners();
    }
  }
